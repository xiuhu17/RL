...
class MegatronPolicyWorkerImpl(AbstractPolicyWorker, ColocatablePolicyInterface):
    ...
    @torch.no_grad()
    @wrap_with_nvtx_name("megatron_policy_worker/connect_sglang_rollout_engines")
    def connect_sglang_rollout_engines(
        ...
        *,
        ...
        Must be called collectively by every Megatron rank when SGLang
        engines are added or recovered. Subsequent calls with the same
        layout are no-ops.
        """
...
def refit_sglang_colocated(
    *,
    ...
    Wraps the Miles-style lifecycle: connect (when new/recovered engines),
    pause + flush, send HF tensor buckets via Ray IPC, post-process, continue.
    """
    from nemo_rl.models.policy.utils import (
    ...
    policy_generation.pause_generation()
